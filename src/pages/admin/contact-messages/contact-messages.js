...
export class AdminContactMessagesPage {
            ...
            <form id="filter-form" class="row g-3">
              <div class="col-md-3">
                <select class="form-select" id="filter-status" name="status">
                  ...
                  <option value="spam" ${this.filters.status === 'spam' ? 'selected' : ''}>Спам</option>
                </select>
              </div>
              <div class="col-md-3">
                  ...
                  <tr class="status-${submission.status}">
                    ...
                    <td>
                      <div class="quick-actions">
                        ...
                        ` : ''}
                        ${submission.status !== 'resolved' && submission.status !== 'spam' ? `
                          <button class="btn btn-outline-success btn-resolve" data-id="${submission.id}" title="Реши">
                            <i class="bi bi-check2-all"></i>
                          </button>
                        ` : ''}
                        ${submission.status !== 'spam' ? `
                          <button class="btn btn-outline-dark btn-spam" data-id="${submission.id}" title="Спам">
  ...
  attachEventListeners() {
    ...
    if (clearFiltersBtn) {
      clearFiltersBtn.addEventListener('click', () => {
        window.router.navigate('/admin/contact-messages');
      });
    }
    ...
    // Spam buttons
    document.querySelectorAll('.btn-spam').forEach(btn => {
      btn.addEventListener('click', () => this.markAsSpam(btn.dataset.id));
    });
  ...
  applyFilters() {
    ...
    window.router.navigate('/admin/contact-messages', params);
  }
    ...
    try {
      ...
      modalBody.innerHTML = `
        <div class="row g-3">
          ...
          ${submission.admin_notes ? `
            <div class="col-12">
              <div class="admin-notes">
                <div class="contact-detail-label">Бележки на администратора</div>
          ...
          <div class="col-12">
            <hr>
            <label class="form-label">Добави/редактирай бележки:</label>
            <textarea class="form-control" id="admin-notes" rows="3" placeholder="Бележки за вътрешна полза...">${this.escapeHtml(submission.admin_notes || '')}</textarea>
          </div>
          <div class="col-12">
            <div class="action-buttons d-flex gap-2 flex-wrap">
              ...
              ` : ''}
              ${submission.status !== 'in_progress' && submission.status !== 'resolved' && submission.status !== 'spam' ? `
                <button class="btn btn-warning btn-modal-progress" data-id="${submission.id}">
                  <i class="bi bi-hourglass-split me-1"></i>В процес
                </button>
              ` : ''}
              ${submission.status !== 'spam' ? `
                <button class="btn btn-dark btn-modal-spam" data-id="${submission.id}">
                  <i class="bi bi-x-octagon me-1"></i>Спам
                </button>
              ` : ''}
            </div>
          </div>
      ...
      const modalSpamBtn = modalBody.querySelector('.btn-modal-spam');
      ...
      if (modalSpamBtn) {
        modalSpamBtn.addEventListener('click', async () => {
          const notes = document.getElementById('admin-notes').value;
          await this.updateStatusWithNotes(submission.id, 'spam', notes);
          this.closeModalAndView();
        });
      }
  ...
  async markAsSpam(submissionId) {
    if (!confirm('Сигурни ли сте, че искате да маркирате това съобщение като спам?')) return;

    try {
      await contactService.updateSubmissionStatus(submissionId, 'spam');
      Toast.success('Съобщението е маркирано като спам!');
      await this.render();
    } catch (error) {
      Toast.error('Грешка при обновяване на съобщението.');
    }
